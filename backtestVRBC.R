...
medal_counts = read_csv(file="./medal_counts.csv")

#data_pred$NOC[which(data_pred$NOC=="AIN")]="RUS"
data_pred = data_scores |>
  filter(Year==2024)
...
for(i in 1:length(innovationP$NOC)){
  ...
  diff = data_scores$standardized[which(data_scores$NOC==curr_NOC & data_scores$Year==2020)] -
    data_scores$standardized[which(data_scores$NOC==curr_NOC & data_scores$Year==2016)]
  if(length(diff)==0){
    diff = 0.1*maxS
  }
...
data_pred$Total_Final_Score[which(data_pred$Total_Final_Score<0)]=0
# use 2024 athlete counts to standardize
for(i in 1:length(std28)){
  curr_NOC=data_pred$NOC[i]
  numAthletes = length(unique(data_athletes$Name[which(data_athletes$NOC==curr_NOC & data_athletes$Year==2020)]))
  std28[i]=data_pred$Total_Final_Score[i]/numAthletes
}
data_pred = data_pred |>
...
carrying_capacity = data.frame("NOC"=data_pred$NOC,"K"=rep(NA,length(data_pred$NOC)))


carrying_capacity <- data_athletes |>
  filter(Year == 2020) |>                  # Filter for the year 2024
...
recent_medals <- medal_counts %>% filter(Year %in% c(2016, 2020))
